...
uint32_t get_mcycleh(void) {
  uint32_t result;
  __asm__ volatile("csrr %0, mcycleh;" : "=r"(result));
  return result;
}

uint64_t get_mcycle64(void) {
  uint32_t reg_high = get_mcycleh();
  uint32_t reg_low = get_mcycle();

  uint64_t result = ((uint64_t)reg_high << 32) | reg_low;
  return result;
}
...
void reset_mcycle64(void) {
  __asm__ volatile("csrw mcycleh, x0");
  __asm__ volatile("csrw mcycle, x0");
}
...
int install_exception_handler(uint32_t vector_num, void(*handler_fn)(void)) {
  ...
  //puts("jmp_ins is 0x"); puthex((uint32_t)jmp_ins); puts("\n");
