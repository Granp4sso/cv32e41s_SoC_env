...
void printNode(const grt_node*);
void printLimitNode(const limit_node*);
void printSizeBits(const uint32_t, const uint8_t);
...
// WARNING: A node is considered valid only if it has children or it is linked to a Limit Table